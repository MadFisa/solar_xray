    ...
    for instrument_i in instruments_list:
        ...
        if not os.path.isfile(fit_file):
            ...
            if instrument_i.name == "daxss":
                instrument_i.set_pha_files(
                    instrument_i.PHA_file_list,
                    ["USE_DEFAULT"] * len(instrument_i.PHA_file_list),
                )
                instrument_i.fit(chisoth_2T, model_args, daxss_fit_args)
            if instrument_i.name == "xsm":
                # instrument_i.arf_file_list = glob.glob(f"{flare_dir}/orig_pha/*.arf")
                instrument_i.set_pha_files(
                    instrument_i.PHA_file_list, instrument_i.arf_file_list
                )
